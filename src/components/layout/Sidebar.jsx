      ...
      <div className="app-sidebar-inner">
        ...
        <nav className="sidebar-nav" aria-label="功能分组">
          {SIDEBAR_GROUPS.map((group) => {
            ...
            const hideSingletonTitle = isSingleton && group.label === group.items[0]?.label
            ...
            if (isSingleton) {
              return (
                <div
                  key={group.id}
                  className={`sidebar-group sidebar-group--singleton ${hideSingletonTitle ? 'sidebar-group--single-action' : ''}`}
                >
                  {!hideSingletonTitle && (
