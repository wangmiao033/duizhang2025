...
function ReconciliationLineItemsForm({
  ...
  useEffect(() => {
    ...
    if (prefillLines.length > 0) {
      const prefillMonth = normalizeSettlementCycleLabel(
        quickFillData.settlementMonth || prefillLines[0]?.settlementCycle || settlementMonth || initialCycle
      )
      setHeader((h) => ({
        ...h,
        settlementMonth: prefillMonth || h.settlementMonth,
        channelFeeRate: quickFillData.channelFeeRate || h.channelFeeRate
      }))
      setLines(
        prefillLines.map((line, index) => ({
          ...createEmptyRdLine(index, prefillMonth || settlementMonth || initialCycle || ''),
          ...line,
          id: `prefill-${Date.now()}-${index}`,
          settlementCycle: normalizeSettlementCycleLabel(line.settlementCycle || prefillMonth),
          sortOrder: index
        }))
      )
      return
